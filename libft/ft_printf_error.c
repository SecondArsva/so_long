...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2023/06/26 23:26:56 by davidga2          #+#    #+#             */
/*   Updated: 2023/06/26 23:44:02 by davidga2         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
{
	if (str[i] == 'c')
		ft_putchar_count_error(va_arg(args, int), ctr);
	else if (str[i] == 's')
		ft_putstr_count_error(va_arg(args, char *), ctr);
	else if (str[i] == 'p')
		ft_putnbr_base_error(va_arg(args, size_t), HEX_BASE_L, 1, ctr);
	else if (str[i] == 'd' || str[i] == 'i')
		ft_putnbr_count_error(va_arg(args, int), ctr);
	else if (str[i] == 'u')
		ft_putnbr_base_error(va_arg(args, unsigned int), DEC_BASE, 0, ctr);
	else if (str[i] == 'x')
		ft_putnbr_base_error(va_arg(args, unsigned int), HEX_BASE_L, 0, ctr);
	else if (str[i] == 'X')
		ft_putnbr_base_error(va_arg(args, unsigned int), HEX_BASE_U, 0, ctr);
	else if (str[i] == '%')
...
static void	ft_spec_format(const char *str, va_list args, int *ctr, size_t i)
{
	while (str[i] != '\0')
	{
		if (str[i] == '%' && str[i + 1] != '\0')
		{
			...
			ft_putchar_count_error(str[i++], ctr);
	}
}
